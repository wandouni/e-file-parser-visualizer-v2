...
export default async function AdminDashboard() {
  ...
  const statCards = [
    { label: '注册用户', value: stats.totalUsers, sub: `+${stats.recentUsers} 近 7 天`, color: '#2563eb' },
    { label: '案例总数', value: stats.totalCases, sub: `+${stats.recentCases} 近 7 天`, color: '#7c3aed' },
    { label: '数据集总数', value: stats.totalHistories, sub: '所有案例', color: '#059669' },
    { label: '封禁用户', value: stats.bannedUsers, sub: '当前封禁', color: '#dc2626' },
  ]
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        {statCards.map(({ label, value, sub, color }) => (
          <div key={label} style={{
            ...
            <p style={{ fontSize: 28, fontWeight: 700, color, marginBottom: 4, lineHeight: 1 }}>
